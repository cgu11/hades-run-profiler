...
class RawSaveFile:
    ...
    def from_file(cls, path: str) -> "RawSaveFile":
        with open(path, "rb") as f:
            ...
            return RawSaveFile(version, dict(parsed_schema.save_data.value))
    ...
    def to_file(self, path: str) -> None:
        if self.version == 14:
            sav14_schema.build_file(
                {
                    "save_data": {
                        "data": rpad_bytes(
                            sav14_save_data_schema.build(self.save_data),
            ...
            sav15_schema.build_file(
                {
                    "save_data": {
                        "data": rpad_bytes(
                            sav15_save_data_schema.build(self.save_data),
            ...
            sav15_schema.build_file(
                {"save_data": {"data": sav16_save_data_schema.build(self.save_data)}},
                filename=path,
            )
        else:
