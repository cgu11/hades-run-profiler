...
class LuaState:
    def __init__(self, version: int, raw_lua_state: List[Dict[Any, Any]]):
    ...
    def from_bytes(cls, version: int, input_bytes: bytes) -> "LuaState":
        ...
        if version == 15:
            decompressed_bytes: bytes = lz4.block.decompress(
                input_bytes, uncompressed_size=SAV15_UNCOMPRESSED_SIZE
            )
        elif version == 16:
            decompressed_bytes: bytes = lz4.block.decompress(
                input_bytes, uncompressed_size=SAV16_UNCOMPRESSED_SIZE
            )

        return LuaState.from_dict(version, decode_luabins(BytesIO(decompressed_bytes)))

    @classmethod
    def from_dict(cls, version: int, input_dicts: List[Dict[Any, Any]]) -> "LuaState":
        return LuaState(version, input_dicts)
    ...
    def _parse_nested_path_reference(self, path: str) -> (Dict[Any, Any], str):
    ...
    def to_dicts(self) -> List[Dict[Any, Any]]:
        return [copy.deepcopy(self._active_state)]
